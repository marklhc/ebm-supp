...
# get_waldci_tau <- function(x) {
#     vc_est <- Cv_to_Vv(x@theta, s = sigma(x))
#     dd2 <- devfun2(x, transfuns = list(from.chol = Cv_to_Sv,
#                                        to.chol = Vv_to_Cv, to.sd = sqrt))
#     var_sig2 <- diag(Matrix::solve(
#         numDeriv::hessian(
#             dd2, vc_est,
#             method.args = list(eps = 1e-10, d = 0.01, r = 6)
#         )))[c(1, 3)] * 2
#     vc_ci <- cbind(
#         "2.5 %" = vc_est[c(1, 3)] - qnorm(.975) * sqrt(var_sig2),
#         "97.5 %" = vc_est[c(1, 3)] + qnorm(.975) * sqrt(var_sig2)
#     )
#     rownames(vc_ci) <- c(".sig01", ".sig03")
#     vc_ci
# }
...
Analyse_ebmc_fp <- function(condition, dat, fixed_objects = NULL) {
    Analyse_ebmc(condition, dat, ci_method = "Wald", REML = FALSE, fp = TRUE)
}
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
    hybrid_mx <- fixed_objects$hybrid_mx
    rep <- condition$REPLICATION
    # Initialize output
    methods <- c("mlm", "cmc", "acmc", "ebmc", "lmc", "lmc_mplus", "lmc_bayes")
    stats <- c("est", "ll", "ul")
    pars <- c("gamma1", "gamma2", "tau0sq", "tau1sq")
    ...
    startvals <- condition[c("gamma1", "gamma2", "tau0sq", "tauxsq")]
    ret$lmc <- tryCatch(
        run_lmc(dat, hybrid_mx = hybrid_mx, startvals = startvals),
        error = function(cnd) {
            error_msgs <<- append(error_msgs, "run_lmc() failed")
            NULL
        }
    )
    # Latent-mean centering (Mplus)
    ret$lmc_mplus <- tryCatch(
        run_lmc_mplus(dat, rep = rep),
        error = function(cnd) {
            error_msgs <<- append(error_msgs, "run_lmc_mplus() failed")
            NULL
        }
    )
    # Latent-mean centering (Mplus, Bayes)
    ret$lmc_bayes <- tryCatch(
        run_lmc_bayes(dat, rep = rep),
        error = function(cnd) {
            error_msgs <<- append(error_msgs, "run_lmc_bayes() failed")
            NULL
        }
    )
...
# Test:
# Analyse(DESIGNFACTOR[29, ], dat = test_dat,
#         fixed_objects = list(hybrid_mx = HYBRID_MX))
...
coverage <- function(cis, par, ci_width = FALSE, names = NULL,
    ...
    if (ci_width & trim == 0) {
        ...
        ret <- apply(cis[, evencs, drop = FALSE] -
...
Evaluate <- function(condition, results, fixed_objects = NULL) {
    methods <- c("cmc", "ebmc_fp", "ebmc_wald", "lmc_mplus")
    # stats <- c("est", "ll", "ul")
    pars <- c("gamma1", "gamma2", "tau0sq", "tau1sq")
    pop <- c(
        condition$gamma1, condition$gamma2,
        condition$tau0sq, condition$tau0sq / 4
    )
    ...
    ret <- c(
        ...
        rmse = tr_rmse(results_est, pop, trim = 0),
...
res <- runSimulation(
    ...
    replications = 20,
    generate = Generate,
    analyse = list(cmc = Analyse_cmc,
                #    ebmc = Analyse_ebmc,
                   ebmc_fp = Analyse_ebmc_fp,
                   ebmc_wald = Analyse_ebmc_wald,
                #    ebmc_ba = Analyse_ebmc_ba,
                   lmc_mplus = Analyse_mplus),
    summarise = NA,
    packages = c("lme4", "bootmlm"),
    fixed_objects = list(
        xcpop = XCPOP
    ),
    filename = "sim_revise_100fp2-results",
    seed = rep(23506100:23506199,
               nrow(DESIGNFACTOR) / 100),
    extra_options = list(allow_na = TRUE,
    ...
    save_results = TRUE,
    save_details = list(
        save_results_dirname = "sim_revise_100fp2-results_"
    )
)
